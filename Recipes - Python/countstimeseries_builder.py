...
class TimeSeriesBuilder(object):
    ...
    def to_series(self):
        ...
        assert ser.sum() == sum(self._counts.itervalues())
        return ser
...
def main():
    ...
    vts = TimeSeriesBuilder()
    vts.increment_at(datetime(2016, 1, 1,  8, 30))
    vts.increment_at(datetime(2016, 1, 1,  8, 45))
    vts.increment_at(datetime(2016, 1, 1,  9, 00))
    print vts.to_series()

    vts = TimeSeriesBuilder()
    vts.increment_at(datetime(2016, 1, 1,  8, 0), "a")
    vts.increment_at(datetime(2016, 1, 1,  8, 30), "a")
    vts.increment_at(datetime(2016, 1, 1,  8, 45), "a")
    vts.increment_at(datetime(2016, 1, 1,  9, 00), "a")
    print vts.to_series()
